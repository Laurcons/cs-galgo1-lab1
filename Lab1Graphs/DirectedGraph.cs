...
namespace Lab1Graphs {
	...
	public class DirectedGraph<TVertex, TCost> : IDirectedGraph<TVertex, TCost> {
		...
		public DirectedGraph() {
			_in = new Dictionary<TVertex, List<TVertex>>();
			_out = new Dictionary<TVertex, List<TVertex>>();
			_costs = new Dictionary<(TVertex, TVertex), TCost>();
		}

		public DirectedGraph(List<TVertex> vertices, List<(TVertex, TVertex, TCost)> edges) {
		...
		public bool IsEdge(TVertex start, TVertex end) {
			if (!IsVertex(start))
				return false;
			return _out[start].Contains(end);
		}
